...
app.config(function ($routeProvider){
	$routeProvider
		...
		{
			controller: '',
			templateUrl: 'partials/onlinegamelobby.html'
		})
		.when('/random/:gameId',
		{
			controller: '',
...
app.controller('friendgameController', function($scope, fbURL, $firebase, $location, $routeParams, $window){
	...
	if(absURL.indexOf("friend")!= -1){
		...
		gameInit.on('child_removed', function(snapshot){
			alert("Error: Your opponent has left the game!");
		}, function(err){
			alert("Error: Some one has left the game!");
		});
		$scope.resetAll();
...
app.controller('onlineHandler', function($scope, fbURL, $firebase, $location, $routeParams, $window){ 
	var absURL = $location.absUrl();
	var targetPairedID;
	if(absURL.indexOf("random")!= -1){
		var randomGameID = Math.floor(Math.random() * 99999).toString();
		fbURL = fbURL + '/randomgames/waitinglist';
		var waitingList =new Firebase(fbURL);
		//waitingList.child(randomGameID).set(randomGameID);
		
		waitingList.on('value', function(data){
			if(data.val() === null){
				console.log('List item does not exist. Create a new list and a new item. ');
				waitingList.child(randomGameID).set(randomGameID);
			} 
			else{
				data.forEach(function(child){
					console.log(child.exportVal());
					targetPairedID = child.val();
					
				});
				//console.log("targetPairedID = " + targetPairedID);
				$location.path("/random/" + targetPairedID);
			}
		});
		//console.log(targetPairedID);
		$location.path("/random/" + targetPairedID);
	}
});

app.controller('onlineController', function($scope, fbURL, $firebase, $location, $routeParams, $window){ 
	this.board  = board;
	this.bigPad = pad;
	$scope.turn_1 = true;
	this.winnerDeclared = false;
	...
	if(absURL.indexOf("random")!= -1){
		...
		fbURL = fbURL + '/randomgames' + '/' +$routeParams.gameId;
		$scope.tttData = $firebase(new Firebase(fbURL));
